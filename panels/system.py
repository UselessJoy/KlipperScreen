...
class Panel(ScreenPanel):
    ...
    def show_update_info(self, widget, program):
        ...
        vbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, valign=Gtk.Align.CENTER, halign=Gtk.Align.CENTER)
        ...
        if program == "full":
            ...
            if not info['is_valid'] or info['is_dirty']:
                ...
                for c in info['commits_behind']:
                    ...
                    title = Gtk.Label(wrap=True, wrap_mode=Pango.WrapMode.CHAR, halign=Gtk.Align.START)
                    title.set_markup(f"\n<b>{c['subject']}</b>\n<i>{c['author']}</i>\n")
                    commit_box.add(title)

                    
                    details = Gtk.Label(label=c['message'], wrap=True, hexpand=True, halign=Gtk.Align.START)
            ...
            vbox.add(label)
            grid = Gtk.Grid(column_homogeneous=True, valign=Gtk.Align.CENTER, halign=Gtk.Align.CENTER)
            i = 0
