...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        self.buttons = {
            ...
            'load': self._gtk.Button("arrow-down", _("Load\nfull"), "color3"),
            'unload': self._gtk.Button("arrow-up", _("Unload\nfull"), "color2"),
    ...
    def load_unload(self, widget, direction):
        if direction == "-":
            if not self.unload_filament:
                self._screen.show_popup_message(_("Macro UNLOAD_FILAMENT not found"))
            else:
                self._screen._ws.klippy.gcode_script(f"UNLOAD_FILAMENT SPEED={self.speed * 60}")
        if direction == "+":
            if not self.load_filament:
                self._screen.show_popup_message(_("Macro LOAD_FILAMENT not found"))
            else:
